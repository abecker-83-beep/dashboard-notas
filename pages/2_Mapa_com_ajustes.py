...
def carregar_dados():
    ...
    if "Status" not in df.columns or df["Status"].isna().all():
        df["Status"] = df["Dias"].apply(lambda x: "Atrasado" if x > 0 else ("Vence hoje" if x == 0 else "No prazo"))
    else:
        df["Status"] = (
            df["Status"].astype(str).str.strip().replace("", np.nan).fillna(
...
def calcular_center(df_mapa):
    ...
    return {"lat": float(df_mapa["lat"].mean()), "lon": float(df_mapa["lon"].mean())}
...
def gerar_base_mapa(df_filtrado, cidades):
    mapa_base = df_filtrado.merge(cidades[["Cidade", "UF", "lat", "lon"]], on=["Cidade", "UF"], how="left")
...
def gerar_agregado_uf(df_filtrado):
    ...
    mapa_uf = (
        ...
        .agg(qtd_nfs=("NF", "count"), valor_total=("Valor", "sum"), vol_total=("Vol", "sum"), qtd_atrasadas=("flag_atraso", "sum"))
        .reset_index()
    )
    mapa_uf["perc_atraso"] = np.where(mapa_uf["qtd_nfs"] > 0, (mapa_uf["qtd_atrasadas"] / mapa_uf["qtd_nfs"]) * 100, 0)
    return mapa_uf
...
def gerar_agregado_cidade(mapa_ok):
    ...
    mapa_cidade["perc_atraso"] = np.where(mapa_cidade["qtd_nfs"] > 0, (mapa_cidade["qtd_atrasadas"] / mapa_cidade["qtd_nfs"]) * 100, 0).round(1)
...
def gerar_mapa_uf(mapa_uf, metrica, metrica_label):
    ...
    fig_uf = px.choropleth(
        ...
        color=metrica,
        hover_name="UF",
        hover_data={"qtd_nfs": True, "valor_total": ":,.2f", "vol_total": True, "qtd_atrasadas": True, "perc_atraso": ":.1f"},
...
def gerar_mapa_cidade_agrupado(mapa_cidade, metrica, metrica_label, map_style, zoom, center, destacar_so_atrasos):
    ...
    fig = px.scatter_mapbox(
        ...
        hover_name="Cidade",
        hover_data={"UF": True, "qtd_nfs": True, "valor_total": ":,.2f", "vol_total": True, "qtd_atrasadas": True, "perc_atraso": ":.1f", "lat": False, "lon": False, "status_mapa": True},
        size_max=34,
        zoom=zoom,
        center=center,
        height=700,
    ...
    fig.update_layout(mapbox_style=map_style, legend_title="Status", margin=dict(l=0, r=0, t=55, b=0))
    return fig


def gerar_mapa_nf_individual(mapa_ok, map_style, zoom, center, destacar_so_atrasos):
    base = mapa_ok.copy()
    if destacar_so_atrasos:
    ...
    fig = px.scatter_mapbox(
        ...
        color_discrete_map=STATUS_COLORS,
        hover_name="Cidade",
        hover_data={"UF": True, "NF": True, "Cliente": True, "Transportadora": True, "Representante": True, "Valor": ":,.2f", "Vol": True, "Dias": True, "Status": True, "lat_plot": False, "lon_plot": False},
        zoom=zoom,
        center=center,
        height=740,
        title="Mapa por NF Individual",
    )
    fig.update_traces(marker=dict(size=8, opacity=0.50))
    fig.update_layout(mapbox_style=map_style, legend_title="Status", margin=dict(l=0, r=0, t=55, b=0))
    return fig, base
...
with c6:
    modo_mapa = st.radio("Visualização", ["Agrupado por cidade", "Cada NF individual"], horizontal=True)
with c7:
...
with k3:
    card_kpi("Cidades no filtro", str(df_filtrado[["Cidade", "UF"]].drop_duplicates().shape[0]), CORES["cinza"])
with k4:
...
perc_coord = percentual(mapa_ok[["Cidade", "UF"]].drop_duplicates().shape[0], mapa_base[["Cidade", "UF"]].drop_duplicates().shape[0])
...
with st.expander("Qualidade geográfica / cidades sem coordenadas"):
    ...
    q1.metric("Cidades com coordenadas", mapa_ok[["Cidade", "UF"]].drop_duplicates().shape[0])
    q2.metric("Cidades sem coordenadas", faltando.shape[0])
    q3.metric("Linhas aptas ao mapa", len(mapa_ok))
...
cidades_criticas = mapa_cidade[mapa_cidade["qtd_atrasadas"] > 0].copy().sort_values(["qtd_atrasadas", "perc_atraso", "valor_total"], ascending=[False, False, False]).head(5)
ufs_criticas = mapa_uf[mapa_uf["qtd_atrasadas"] > 0].copy().sort_values(["qtd_atrasadas", "perc_atraso", "valor_total"], ascending=[False, False, False]).head(5)
...
with st.expander("Ver ranking por UF"):
    st.dataframe(ranking_uf[["UF", "qtd_nfs", "valor_total_fmt", "vol_total", "qtd_atrasadas", "perc_atraso"]], use_container_width=True, hide_index=True)

st.divider()
st.subheader("📍 Mapa Analítico")

if modo_mapa == "Agrupado por cidade":
    fig_cidade = gerar_mapa_cidade_agrupado(mapa_cidade, metrica, metrica_label, estilo_mapa, zoom, center, destacar_so_atrasos)
    ...
    ranking_cidade = ranking_cidade.sort_values([metrica, "qtd_atrasadas", "perc_atraso"], ascending=[False, False, False])
    ranking_cidade["valor_total_fmt"] = ranking_cidade["valor_total"].apply(formatar_moeda_br)

    with st.expander("Ver ranking por cidade"):
        st.dataframe(ranking_cidade[["Cidade", "UF", "qtd_nfs", "valor_total_fmt", "vol_total", "qtd_atrasadas", "perc_atraso", "status_mapa"]], use_container_width=True, hide_index=True)
else:
    fig_nf, mapa_nf = gerar_mapa_nf_individual(mapa_ok, estilo_mapa, zoom, center, destacar_so_atrasos)
    if fig_nf is None:
        st.warning("Nenhuma NF encontrada para exibir no modo individual com o filtro atual.")
    else:
        st.plotly_chart(fig_nf, use_container_width=True, config={"scrollZoom": True})

    with st.expander("Ver tabela detalhada das NFs no mapa"):
        tabela_nf = mapa_nf[["NF", "Cidade", "UF", "Cliente", "Transportadora", "Representante", "Valor", "Vol", "Dias", "Status"]].copy()
        tabela_nf["Valor"] = tabela_nf["Valor"].apply(formatar_moeda_br)
        st.dataframe(tabela_nf.sort_values(["UF", "Cidade", "Status"]), use_container_width=True, hide_index=True)
